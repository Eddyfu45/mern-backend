...
const show = async (req, res) => {
    ...
    try {
        ...
        console.log(error);
        res.status(400).json({ message: 'Book not found. Try again...' });
    }
}
...
const update = async (req, res) => {
    ...
    try {
        ...
        console.log(error);
        return res.status(400).json({ message: 'Book cannot be updated. Please try again...' });
    }
}
...
// GET -> api/books/
router.get('/', passport.authenticate('jwt', {session:false }), index); // this route is currently not protected
// GET -> /api/books/:id
